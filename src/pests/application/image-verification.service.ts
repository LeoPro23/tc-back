...
export class ImageVerificationService {
  ...
  async verifyImage(
    imageBuffer: Buffer,
    mimeType: string,
  ): Promise<ImageVerificationDecision> {
  ...
  private async checkImageQuality(
    ...
    try {
      ...
      this.logger.debug(
        `Calidad de imagen - Brillo: ${avgBrightness.toFixed(1)}, Saturación: ${avgSaturation.toFixed(1)}`,
      );
      ...
      this.logger.warn(
        `No se pudo analizar calidad de imagen: ${error.message}. Continuando con verificación LLM.`,
      );
      return {
        isValid: true,
        reason: 'No se pudo verificar calidad, se acepta por defecto',
      };
    }
  }
  ...
  private getValidatedBaseUrl(): string {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      throw new InternalServerErrorException(
        'OPENROUTER_BASE_URL must use http or https',
      );
    }
  ...
  private parseDecision(rawContent: unknown): ImageVerificationDecision {
    if (typeof rawContent !== 'string' || rawContent.trim().length === 0) {
      throw new InternalServerErrorException(
        'Empty verification response from OpenRouter',
      );
    }
    ...
    if (!jsonCandidate) {
      throw new InternalServerErrorException(
        'Invalid verification format from OpenRouter',
      );
    }
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to parse OpenRouter verification JSON',
      );
    }

    const isValid = this.parseBoolean(
      parsed.is_valid ?? parsed.isValid ?? parsed.valid,
    );
    const reason =
      typeof parsed.reason === 'string' && parsed.reason.trim().length > 0
        ? parsed.reason.trim()
        : isValid
          ? 'imagen aprobada para análisis de plagas'
          : 'imagen rechazada por no corresponder al dominio';
  ...
  private parseBoolean(value: unknown): boolean {
    ...
    if (typeof value === 'string') {
      ...
      if (
        ['true', 'yes', 'si', 'sí', 'valid', 'relevant', '1'].includes(
          normalized,
        )
      ) {
        return true;
    ...
    throw new InternalServerErrorException(
      'Invalid boolean value in verification response',
    );
  }
}
