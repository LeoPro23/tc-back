...
export class AnalysisInterpretationService {
  ...
  private mergeWithFallback(
    parsed: BatchInterpretation & {
      perImage: Partial<PerImageInterpretation>[];
    },
    results: PestAnalysisResult[],
  ): BatchInterpretation {
    ...
    const mergedPerImage = results.map((result, index) => {
      ...
      const byFilename = perImageByFilename.get(
        this.normalizeFilename(result.filename) ?? '',
      );
      ...
      return {
        filename: result.filename,
        targetPest: this.asText(
          candidateRaw.target_pest ?? candidateRaw.targetPest,
          base.targetPest,
        ),
        imageRecommendation: this.asText(
          candidateRaw.image_recommendation ?? candidateRaw.imageRecommendation,
          base.imageRecommendation,
        ),
        recipe: {
          product: this.asText(
            candidateRaw.recipe?.product,
            base.recipe.product,
          ),
    ...
    return {
      generalSummary: this.asText(
        parsed.generalSummary,
        fallback.generalSummary,
      ),
      generalRecommendation: this.asText(
        parsed.generalRecommendation,
        fallback.generalRecommendation,
      ),
      generalProduct: this.asText(
        parsed.generalProduct,
        fallback.generalProduct,
      ),
      generalOperativeGuide: this.asText(
        parsed.generalOperativeGuide,
        fallback.generalOperativeGuide,
      ),
      generalBiosecurityProtocol: this.asText(
        parsed.generalBiosecurityProtocol,
        fallback.generalBiosecurityProtocol,
      ),
  ...
  private parseRawResponse(
    rawContent: unknown,
  ): BatchInterpretation & { perImage: Partial<PerImageInterpretation>[] } {
    ...
    const generalRecommendation = this.asText(
      parsed.general_recommendation ?? parsed.generalRecommendation,
      'Aplicar medidas preventivas básicas.',
    );
    const generalProduct = this.asText(
      parsed.general_product ?? parsed.generalProduct,
      'Preventivo genérico',
    );
    const generalOperativeGuide = this.asText(
      parsed.general_operative_guide ?? parsed.generalOperativeGuide,
      'Monitorear cultivo de forma regular.',
    );
    const generalBiosecurityProtocol = this.asText(
      parsed.general_biosecurity_protocol ?? parsed.generalBiosecurityProtocol,
      'Mantener protocolos estándar de bioseguridad.',
    );
  ...
  private buildFallbackInterpretation(
    results: PestAnalysisResult[],
  ): BatchInterpretation {
    ...
    return {
      generalSummary,
      generalRecommendation:
        'Evaluación rápida de lote completada por motor ML.',
      generalProduct: 'No aplica',
      generalOperativeGuide:
        'Seguir lineamientos preestablecidos por equipo agrónomo local.',
      generalBiosecurityProtocol:
        'Aplicar normas de la finca. Precaución preventiva ante cualquier síntoma visual.',
      perImage: results.map((result) => this.buildFallbackPerImage(result)),
    };
  }

  private buildFallbackPerImage(
    ...
    if (!result.verified) {
      return {
        ...
        imageRecommendation:
          'La imagen fue rechazada por falta de foco agrícola.',
        recipe: {
          product: 'No aplica',
          dose: 'No aplica',
          method:
    ...
    if (result.detections.length === 0) {
      return {
        ...
        recipe: {
          ...
          dose: 'No aplica',
          method:
            'Mantener monitoreo semanal y continuar buenas prácticas agronómicas.',
        },
